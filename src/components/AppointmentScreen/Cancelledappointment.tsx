...
const Cancelledappointment = () => {
  ...
  useEffect(() => {
    const fetchCancelledAppointments = async () => {
      ...
      if (result && result.data) {
        const filtered = result.data.filter((appointment: Appointment) => appointment.appointment_status === 3);
        setCancelledAppointments(filtered);
  ...
  useEffect(() => {
    if (searchTerm === '') {
      ...
      const filtered = cancelledAppointments.filter((appointment) =>
  ...
  }, [searchTerm, cancelledAppointments]);
  ...
  return (
    <Box sx={{ padding: 2 }}>
      ...
      <TableContainer component={Paper} sx={{ maxHeight: '60vh', overflowY: 'auto' }}>
        <Table aria-label="Cancelled appointments">
          <TableHead>
