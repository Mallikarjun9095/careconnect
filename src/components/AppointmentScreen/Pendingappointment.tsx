...
const AppointmentsPending = () => {
  ...
  useEffect(() => {
    const fetchPendingAppointments = async () => {
      ...
      if (result && result.data) {
        const filteredPending = result.data.filter((appointment: Appointment) => appointment.appointment_status === 0);
        setPendingAppointments(filteredPending);
        setFilteredAppointments(filteredPending); 
      }
    };
  ...
  useEffect(() => {
    if (searchTerm === '') {
      ...
      const filtered = pendingAppointments.filter((appointment) => 
  ...
  }, [searchTerm, pendingAppointments]);
  ...
  return (
    <Box sx={{ padding: 2 }}>
      ...
      <TableContainer component={Paper} sx={{ maxHeight: '60vh', overflowY: 'auto' }}>
        <Table aria-label="pending appointments">
          <TableHead>
